...
const Multiplefromdownload = (JSONData, ReportTitle, ShowLabel, trackName) => {
    var arrData = [];
    ...
    arrData.forEach(function (objectval, index) {
        ...
        CSV += row + '\r\n';
        row = "";
        objectval.data.forEach(function (objectval2, index) {

            var row = "";
            row += '"' + objectval2.name + '",';
            row += '"' + objectval2.value + '",';
            row.slice(0, row.length - 1);
            CSV += row + '\r\n';
            row = "";
        });
...
export default Multiplefromdownload;